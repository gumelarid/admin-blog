...
@section('content')
        ...
        <div class="card-body px-0 pb-2">
            ...
            <form action="{{ url('/dashboard/article/update') }}" method="POST" class="p-3" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                  <div class="col-md-8">
                    <div class="input-group input-group-outline my-3">
                      ...
                      <input type="text" name="title" value="{{ $data->title_article }}"  class="form-control">
                    </div>

                    <div class="input-group input-group-dynamic">
                        <textarea id="description" name="description" class="form-control" rows="5" placeholder="Description" spellcheck="false">{{ $data->description }}</textarea>
                    </div>
                  </div>
                  <div class="col-md-4">
                    @if ($data->thumbnail)
                    ...
                    <div class="input-group input-group-static mb-4">
                        ...
                        <select style="padding: 10px" name="category" class="form-control" id="exampleFormControlSelect1" value="{{ $data->category_id }}">
                          <option disabled>Select Category</option>
                          @foreach ($category as $ca)
                            <option value="{{ $ca->category_id }}" {{ ($data->category_id == $ca->category_id) ? "selected" : "" }}>{{ $ca->category }}</option>
                          @endforeach
                        </select>
                      </div>
                    <div class="form-check form-switch">
                        <input name="is_publish" class="form-check-input" type="checkbox" {{ $data->status == 1 ? 'checked' : '' }}>
                        <label class="form-check-label" for="flexSwitchCheckDefault">Publish</label>
                    </div>
                    <hr>

                    <div class="input-group input-group-dynamic">
                        <textarea name="meta" class="form-control" rows="5" placeholder="Meta Description" spellcheck="false">{{ $data->meta_description }}</textarea>
                    </div>
